...
char MxPad::msg_no_error[] =      "No error occurred.";
char MxPad::msg_unknown_error[] = "Unknown error.";
...

{
    ...
    AP2N("After ling sockoption %d\r\n", ret_val);

    ret_val = getsockopt (sock2, SOL_SOCKET, SO_DONTLINGER, &nd, &ndlen);
    
	AP2N("After ling getsockoption %d ---  %d\r\n", ret_val, nd);

    rec_sin.sin_addr.s_addr = INADDR_ANY;
    rec_sin.sin_port = htons(MX_DATA_PORT);
    ...
    if (bind( sock2, (struct sockaddr FAR *) &rec_sin, sizeof(rec_sin))
		...
		TRACE("MxPad::AcceptThreadProc -- Cannot Bind socket\r\n");
		AP2N("MxPad::AcceptThreadProc -- Cannot Bind socket\r\n");
		//Sleep(1000);
        //exit(0);
        }
    ...
    if (listen( sock2, 1) < 0)
		...
		TRACE("MxPad::AcceptThreadProc Cannot listen on socket2\r\n");
        AP2N("MxPad::AcceptThreadProc Cannot listen on socket\r\n");
        //exit(0);
        }
...

{
	...
	//dump_str = (char *) malloc(MXDUMPLEN);	ASSERT(dump_str);
	//memset(dump_str, 'a', DUMPLEN);

	//szOutString = (char *) malloc(MXOUTLEN); ASSERT(szOutString);
	//memset(szOutString, 'a', OUTLEN);

	//aszOutString = (char *) malloc(MXOUTLEN); ASSERT(aszOutString);
	//memset(szOutString, 'a', OUTLEN);
...

{
	...
	if(!constructed)
		{
		TRACE("Warning: Called Mxpad before construction\r\n");
		TRACE("Format str was:'%s'\r\n", Format);
		return 0;
		}

	if(!szOutString)
		{
		TRACE("Warning: Called Mxpad after destruction\r\n");
		TRACE("Format str was:'%s'\r\n", Format);
		return 0;
		}

	//TRACE("MxPad::printf with '%s'\r\n", Format);
...
//		}
	...
	try
		...
		len = _vsnprintf(szOutString, MXOUTLEN - 1, Format, ArgList);
...

{
	...
	if(!dump_str)
		{
		TRACE("Warning: Called Mxpad after destruction\r\n");
		TRACE("Memory dump of mem=%p len=%d\r\n", mem, len);
		return 0;
		}

    if (!mem)
        {
        ToNotepad("MxPad::dump NULL pointer\r\n"); 
		return(0);
        }

	if(!len)
		{
        ToNotepad("MxPad::dump Buffer with zero length.\r\n"); 
		return(0);
        }

	if(!AfxIsValidAddress(mem, len))
		{
		ToNotepad("MxPad::dump Invalid pointer passed.\r\n"); 
		return 0;
		}
...
int     MxPad::ToNotepad(const char  *str)

{
	...
	if(remote)
		...
		if(send(sock2, str, len, 0) < 0)
			...
			//CString str2; str2.Format("Cannot Send '%s'\r\n", str);
			//TRACE(str2);
			}
		}
    return(0);
...

{
	...
	dest_sin.sin_port = htons(MX_CONN_PORT);
	...
	if (connect( sock2, (PSOCKADDR) &dest_sin, sizeof( dest_sin)) < 0)
		...
		TRACE("MxPad::Connect: cannot connect\r\n");
		#endif
...

{
	...
	try
		...
		len = _vsnprintf(aszOutString, MXOUTLEN - 1, Format, ArgList);
		}
...
char *MxPad::sockerr2str(int err)
